...
export const paymentController = {
  ...
  getTestSignature: async (req: Request, res: Response): Promise<void> => {
    try {
      if (razorpayService.getMode() === 'LIVE') {
        res.status(403).json({
          success: false,
  ...
  createOrder: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (type === 'SUBSCRIPTION' || type === 'FAMILY_SUBSCRIPTION') {
        ...
        const plan = db.subscriptionPlans.findOne(
          p => p.planId === targetPlanId || p.id === targetPlanId
        );
        if (!plan) {
        ...
        const bill = db.bills.findOne(
          b => b.billId === referenceId || b.id === referenceId
        );
        if (!bill) {
        ...
        const license = db.organizationLicenses.findOne(
          l => l.licenseId === referenceId || l.id === referenceId
        );
        const plan = db.subscriptionPlans.findOne(
          p => p.planId === (license?.planId || referenceId) || p.id === (license?.planId || referenceId)
        );
        ...
        const usage = db.usageRecords.findOne(
          u => u.id === referenceId || u.id === referenceId.replace('usage-', 'usg-') || u.id === referenceId.replace('usg-', 'usage-')
        );
        if (!usage) {
      ...
      const order = razorpayService.createOrder(
        amountInRupees,
        type === 'SUBSCRIPTION' ? 'sub' : 'bill',
        orderNotes
      );
  ...
  verifyPayment: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const existingSubPayment = await sqliteDb.getSubscriptionPaymentByPaymentId(razorpay_payment_id);
      const existingBillPayment = await sqliteDb.getBillPaymentByPaymentId(razorpay_payment_id);
      const existingOrgPayment = await sqliteDb.getOrganizationPaymentByPaymentId(razorpay_payment_id);
      ...
      if (type === 'SUBSCRIPTION' || type === 'FAMILY_SUBSCRIPTION') {
        const plan = db.subscriptionPlans.findOne(
          p => p.planId === referenceId || p.id === referenceId
        );
        if (!plan) {
        ...
        const existingSub = db.subscriptions.findOne(s => s.patientId === patientId);
        if (existingSub) {
          db.subscriptions.update(existingSub.id, subscriptionRecord);
        } else {
          db.subscriptions.insert(subscriptionRecord);
        }

        // Update SQLite Subscriptions table
        await sqliteDb.upsertSubscription(subscriptionRecord);

        // Record Subscription Payment in SQLite
        const subPaymentId = `SPAY-${Date.now().toString().slice(-6)}`;
        const subPaymentRecord: SubscriptionPayment = {
          id: `spay-${uuidv4().substring(0, 8)}`,
        ...
        await sqliteDb.insertSubscriptionPayment(subPaymentRecord);
        ...
        db.payments.insert({
          ...
          gatewayMode: razorpayService.getMode() === 'TEST' ? 'SANDBOX' : 'LIVE',
          paidAt: now,
        ...
        const billPaymentRecord: BillPayment = {
          id: `bpay-${uuidv4().substring(0, 8)}`,
        ...
        await sqliteDb.insertBillPayment(billPaymentRecord);
        ...
        // Dispatch notification
        const serviceName = bill.items?.[0]?.description || (bill as any).testName || 'medical investigation';
        db.notifications.insert({
          id: `notif-${uuidv4().substring(0, 8)}`,
          patientId: bill.patientId,
          recipientId: user?.id || 'usr-pat-1',
        ...
        res.json({
          ...
          message: `Payment successful for ${serviceName}. Bill ${bill.billId || bill.id} is now paid.`,
          data: {
            bill: db.bills.findById(bill.id),
        ...
        if (license) {
          db.organizationLicenses.update(license.id, {
            status: 'ACTIVE',
            updatedAt: now
          });
        }
        ...
        const orgPaymentRecord: OrganizationPayment = {
          id: `opay-${uuidv4().substring(0, 8)}`,
        ...
        await sqliteDb.insertOrganizationPayment(orgPaymentRecord);
  ...
  getSubscriptionPaymentHistory: async (req: Request, res: Response): Promise<void> => {
    try {
      const patientId = req.query.patientId as string || req.user?.patientId || 'PAT10001';
      const payments = await sqliteDb.getSubscriptionPayments(patientId);
      res.json({
        success: true,
        data: { payments }
      });
  ...
  getBillPaymentHistory: async (req: Request, res: Response): Promise<void> => {
    try {
      const patientId = req.query.patientId as string || req.user?.patientId || 'PAT10001';
      const payments = await sqliteDb.getBillPayments(patientId);
      res.json({
        success: true,
        data: { payments }
      });
  ...
  getFamilyMembers: async (req: Request, res: Response): Promise<void> => {
    try {
      const patientId = req.query.patientId as string || req.user?.patientId || 'PAT10001';
      const members = await sqliteDb.getFamilyMembers(patientId);
      res.json({
        success: true,
        data: { members }
      });
  ...
  addFamilyMember: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const sub = db.subscriptions.findOne(s => s.patientId === patientId && s.status === 'ACTIVE');
      ...
      const existingMembers = await sqliteDb.getFamilyMembers(patientId);
      ...
      const memberId = `FAM-${uuidv4().substring(0, 8)}`;
      ...
      await sqliteDb.addFamilyMember(newMember);

      // Update in-memory subscription record
      db.subscriptions.update(sub.id, {
  ...
  handleWebhook: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const eventId = event.event_id || event.payload?.payment?.entity?.id || uuidv4();
      const now = new Date().toISOString();

      // Check webhook idempotency
      const alreadyProcessed = await sqliteDb.hasWebhookEventBeenProcessed(eventId);
      if (alreadyProcessed) {
      ...
      await sqliteDb.recordWebhookEvent({
      ...
      if (eventType === 'payment.captured' || eventType === 'order.paid') {
        ...
        if (pType === 'BILL' && refId) {
          const bill = db.bills.findOne(b => b.billId === refId || b.id === refId);
          if (bill && bill.status !== 'PAID') {
            ...
            await sqliteDb.insertBillPayment({
              id: `bpay-${uuidv4().substring(0, 8)}`,
          ...
          const plan = db.subscriptionPlans.findOne(p => p.planId === notes.planId);
          if (plan) {
            ...
            const existing = db.subscriptions.findOne(s => s.patientId === notes.patientId);
            if (existing) {
              db.subscriptions.update(existing.id, subRecord);
            } else {
              db.subscriptions.insert(subRecord);
            }
            await sqliteDb.upsertSubscription(subRecord);

            await sqliteDb.insertSubscriptionPayment({
              id: `spay-${uuidv4().substring(0, 8)}`,
  ...
  getOrganizationPaymentHistory: async (req: Request, res: Response): Promise<void> => {
    try {
      const { organizationId } = req.query;
      const payments = await sqliteDb.getOrganizationPayments(organizationId as string | undefined);
  ...
  setRuntimeMode: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      razorpayService.setMode(mode, keyId, keySecret);
      res.json({
        success: true,
