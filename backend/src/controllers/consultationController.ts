...
import { ConsultationRequest, Consultation, Consent, Notification } from '../models/types';
...
export const consultationController = {
  ...
  searchDoctors: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const allDoctors = db.doctors.find();
      ...
      const filtered = doctorCatalog.filter(doc =>
  ...
  requestConsultation: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const existingPending = db.consultationRequests.findOne(
        r => r.patientId === patientId && (r.doctorId === doctor.doctorId || r.doctorId === doctor.id) && r.status === 'PENDING'
      );
      if (existingPending) {
        res.status(400).json({
          success: false,
      ...
      const clinic = db.clinics.findById(doctor.clinicId);
      ...
      db.consultationRequests.insert(newRequest);

      // Notify Doctor
      const doctorUser = db.users.findOne(u => u.doctorId === doctor.doctorId || u.id === doctor.userId);
      if (doctorUser) {
        db.notifications.insert({
          id: `notif-cr-${uuidv4().substring(0, 8)}`,
      ...
      const hospitalRecs = db.receptionists.find(r => r.hospitalId === hospId);
      for (const rec of hospitalRecs) {
        const recUser = db.users.findOne(u => u.id === rec.userId || u.receptionistId === rec.receptionistId);
        if (recUser) {
          db.notifications.insert({
            id: `notif-cr-rec-${uuidv4().substring(0, 8)}`,
  ...
  getPatientConsultations: async (req: Request, res: Response): Promise<void> => {
    try {
      const patientId = req.params.patientId || req.user?.patientId;
      const requests = db.consultationRequests.find(r => r.patientId === patientId)
  ...
  getMyDoctors: async (req: Request, res: Response): Promise<void> => {
    try {
      const patientId = req.params.patientId || req.user?.patientId;
      const patient = db.patients.findOne(p => p.patientId === patientId);
      ...
      const doctors = db.doctors.find(
        d => connectedDoctorIds.includes(d.doctorId) || connectedDoctorIds.includes(d.id)
      ).map(doc => {
        const clinic = clinics.find(c => c.id === doc.clinicId);
  ...
  getMyClinics: async (req: Request, res: Response): Promise<void> => {
    try {
      const patientId = req.params.patientId || req.user?.patientId;
      const patient = db.patients.findOne(p => p.patientId === patientId);
      ...
      const connectedDoctorIds = patient.doctorIds || [];
      const connectedDoctors = db.doctors.find(
        d => connectedDoctorIds.includes(d.doctorId) || connectedDoctorIds.includes(d.id)
      );
      ...
      const clinics = db.clinics.find(c => clinicIds.includes(c.id)).map(clinic => {
        const org = db.organizations.findById(clinic.organizationId);
        return {
          ...clinic,
  ...
  getDoctorRequests: async (req: Request, res: Response): Promise<void> => {
    try {
      const doctorId = req.user?.doctorId || 'DOC10001';
      const requests = db.consultationRequests.find(
  ...
  respondToRequest: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const consultationReq = db.consultationRequests.findById(id);
      if (!consultationReq) {
      ...
      if (action === 'ACCEPTED') {
        ...
        const doctor = db.doctors.findOne(d => d.doctorId === consultationReq.doctorId || d.id === consultationReq.doctorId);
        const patient = db.patients.findOne(p => p.patientId === consultationReq.patientId);
        ...
        if (doctor && patient) {
          ...
          db.consents.insert(consentDoctor);
          ...
          db.consultations.insert(consultationRecord);
        }

        // Notify patient
        const patientUser = db.users.findOne(u => u.patientId === consultationReq.patientId);
        if (patientUser) {
          db.notifications.insert({
            id: `notif-c-acc-${uuidv4().substring(0, 8)}`,
        ...
        if (patientUser) {
          db.notifications.insert({
            id: `notif-c-rej-${uuidv4().substring(0, 8)}`,
            recipientId: patientUser.id,
            recipientRole: 'PATIENT',
            patientId: consultationReq.patientId,
            type: 'FOLLOW_UP',
