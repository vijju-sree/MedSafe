...
import { Prescription, Medication, MedicationSchedule, MedicationSupply, TimeOfDay } from '../models/types';

export const prescriptionController = {
  getPrescriptions: async (req: Request, res: Response): Promise<void> => {
    try {
  ...
  createPrescription: async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const finalMedicationIds: string[] = Array.isArray(medicationIds) ? [...medicationIds] : [];

      // If full medication objects were passed, instantiate them into db.medications
      if (Array.isArray(medications) && medications.length > 0) {
        for (const m of medications) {
          const medId = `med-${uuidv4().substring(0, 8)}`;
      ...
      if (user) {
        db.notifications.insert({
          id: `notif-rx-${uuidv4().substring(0, 8)}`,
          recipientId: user.id,
  ...
  renewPrescription: async (req: Request, res: Response): Promise<void> => {
    try {
      const { id } = req.params;
      const { additionalDays, notes } = req.body;

      const rx = db.prescriptions.findById(id);
      if (!rx) {
