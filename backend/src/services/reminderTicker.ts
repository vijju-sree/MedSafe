...
import { MedicationSchedule, Notification } from '../models/types';

export class ReminderTicker {
  private timer: NodeJS.Timeout | null = null;

  start(): void {
    console.log(`Starting MedSafe Reminder Ticker (interval: ${config.reminderCheckIntervalSeconds}s, grace: ${config.gracePeriodMinutes}m)...`);
    this.checkSchedules();
  ...
  checkSchedules(): void {
    ...
    const schedulesToday = db.medicationSchedules.find(
      s => s.scheduledDate === todayStr && s.status === 'UPCOMING'
    );
    ...
    for (const schedule of schedulesToday) {
      ...
      if (diffMinutes >= 0 && !schedule.reminderSent) {
        db.medicationSchedules.update(schedule.id, { reminderSent: true });
        ...
        if (user) {
          ...
          const existingNotif = db.notifications.findOne(
            n => n.type === 'MEDICATION_REMINDER' && n.metadata?.scheduleId === schedule.id
          );

          if (!existingNotif) {
            const notif: Notification = {
              id: `notif-rem-${uuidv4().substring(0, 8)}`,
              recipientId: user.id,
      ...
      if (diffMinutes > schedule.graceMinutes) {
        ...
        db.medicationSchedules.update(schedule.id, { status: 'MISSED' });

        // Record Adherence
        db.adherenceRecords.insert({
          id: `adh-miss-${uuidv4().substring(0, 8)}`,
          patientId: schedule.patientId,
        ...
        const activeConsents = db.consents.find(
        ...
        for (const consent of activeConsents) {
          ...
          emailService.sendMissedDoseAlert(
            consent.granteeEmail,
            consent.granteeName,
            patient?.name || 'Patient',
            schedule.medicationName,
            schedule.scheduledTime
          ).catch(e => console.error('Caregiver email failed:', e));
        }
        ...
        if (recentMissed.length >= 3) {
          ...
          if (!existingFollowUp) {
            db.followUpActions.insert({
              id: `flw-auto-${uuidv4().substring(0, 8)}`,
              patientId: schedule.patientId,
              medicationId: schedule.medicationId,
              medicationName: schedule.medicationName,
   ...
   */
  triggerTestReminder(patientId: string, medicationId?: string): MedicationSchedule {
    const now = new Date();
    const todayStr = now.toISOString().split('T')[0];
    const hours = String(now.getHours()).padStart(2, '0');
    const minutes = String(now.getMinutes()).padStart(2, '0');

    let med = medicationId ? db.medications.findById(medicationId) : undefined;
    if (!med) {
      med = db.medications.findOne(m => m.patientId === patientId && m.active) || {
        id: 'med-test',
        patientId,
    ...
    const testSchedule: MedicationSchedule = {
      id: `sch-test-${uuidv4().substring(0, 8)}`,
      patientId,
      medicationId: med.id,
    ...
    db.medicationSchedules.insert(testSchedule);
    ...
    if (user) {
      db.notifications.insert({
        id: `notif-test-${uuidv4().substring(0, 8)}`,
        recipientId: user.id,
        recipientRole: 'PATIENT',
        patientId,
        type: 'MEDICATION_REMINDER',
...
export const reminderTicker = new ReminderTicker();
