 ...
 */
export function authorizePatientAccess(paramName: string = 'patientId') {
  return (req: Request, res: Response, next: NextFunction): void => {
    if (!req.user) {
      res.status(401).json({ success: false, message: 'Authentication required.' });
      return;
    ...
    if (req.user.role === 'CAREGIVER') {
      const caregiver = db.caregivers.findOne(c => c.userId === req.user?.id);
      const activeConsent = db.consents.findOne(
        c => c.patientId === patientId && c.granteeId === req.user?.id && c.status === 'ACTIVE'
      );
    ...
    if (req.user.role === 'DOCTOR') {
      const doctor = db.doctors.findOne(d => d.userId === req.user?.id);
      const activeConsent = db.consents.findOne(
        c => c.patientId === patientId && (c.granteeId === req.user?.id || c.granteeId === doctor?.id) && c.status === 'ACTIVE'
      );
    ...
    if (req.user.role === 'PHARMACIST') {
      const pharmacist = db.pharmacists.findOne(p => p.userId === req.user?.id);
