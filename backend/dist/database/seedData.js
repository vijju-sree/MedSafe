...
async function seedDatabase(force = false) {
    ...
    db_1.db.organizations.insertMany([orgHospital, orgClinic, orgPharmacy]);
    const clinicMain = {
        id: 'clin-1',
    ...
    db_1.db.clinics.insertMany([clinicMain, clinicEast]);
    ...
    const recPasswordHash = await bcryptjs_1.default.hash('DemoPass123!', salt);
    // Receptionist Priya Sharma (ABC Hospital)
    const userReceptionistABC = {
        id: 'usr-rec-1',
    ...
    db_1.db.patients.insertMany([patientProfile, patientProfile2, patientProfile3]);
    db_1.db.doctors.insertMany([doctorProfile, doctorProfile2, doctorProfile3, pendingDocProfile]);
    db_1.db.caregivers.insert(caregiverProfile);
    db_1.db.pharmacists.insertMany([pharmacistProfile, pendingPharmProfile]);
    db_1.db.labs.insert(labProfile);
    db_1.db.receptionists.insertMany([receptionistProfileABC, receptionistProfileXYZ]);
    ...
    db_1.db.patientHospitalRelationships.insert(phr1);
    const dpr1 = {
        id: 'dpr-1',
    ...
    db_1.db.doctorPatientRelationships.insert(dpr1);
    ...
    db_1.db.consultationRequests.insertMany([pendingReq, incomingPendingReq]);
    // 3. Prescriptions
    const prescription1 = {
        id: 'rx-1',
    ...
    db_1.db.prescriptions.insertMany([prescription1, prescription2, prescriptionOld]);
    // 4. Medications
    const medParacetamol = {
        id: 'med-1',
        patientId: 'PAT10001',
        prescriptionId: 'RX10001',
        doctorId: 'DOC10001',
    ...
    db_1.db.medications.insertMany([medParacetamol, medVitaminD, medMetformin, medAtorvastatin, medCrocinDuplicate]);
    // 5. Today's Schedules & Recent History
    const scheduleTodayMorning = {
        id: 'sch-today-1',
        patientId: 'PAT10001',
        medicationId: 'med-1',
        prescriptionId: 'RX10001',
    ...
    db_1.db.medicationSchedules.insertMany([
    ...
    db_1.db.adherenceRecords.insertMany(adherenceRecords);
    ...
    db_1.db.safetyAlerts.insertMany([alertDuplicate, alertExpiringRx]);
    // 7. Follow-Up Actions
    const followUpMissedDoses = {
        id: 'flw-1',
        patientId: 'PAT10001',
        medicationId: 'med-3',
        medicationName: 'Metformin 500 mg',
    ...
    db_1.db.followUpActions.insertMany([followUpMissedDoses, followUpRefill]);
    // 8. Consents
    const consentCaregiver = {
        id: 'con-1',
        patientId: 'PAT10001',
        granteeId: 'usr-car-1',
    ...
    db_1.db.consents.insertMany([consentCaregiver, consentDoctor]);
    ...
    db_1.db.medicationReferences.insertMany(drugReferences);
    ...
    db_1.db.auditLogs.insertMany(auditLogs);
    ...
    db_1.db.testOrders.insertMany([order1, order2, order3]);
    // Medical Report
    const report1 = {
        id: 'rpt-1',
        reportId: 'RPT10001',
    ...
    db_1.db.medicalReports.insert(report1);
    // 13. Bills & Payments
    const bill1 = {
        id: 'bil-1',
        billId: 'BIL10001',
    ...
    db_1.db.bills.insertMany([bill1, bill2, bill3]);
    const payment1 = {
        id: 'pay-1',
        paymentId: 'PAY10001',
        billId: 'BIL10001',
    ...
    db_1.db.payments.insert(payment1);
    seedMissingCollections(now, todayStr, addDays);
    console.log('MedSafe demo database successfully seeded!');
}
function seedMissingCollections(now, todayStr, addDays) {
    // A. Subscription Plans
    if (db_1.db.subscriptionPlans.count() === 0) {
        const plans = [
            {
                id: 'plan-free',
                planId: 'PLAN_FREE',
        ...
        db_1.db.subscriptionPlans.insertMany(plans);
    }
    // B. Subscriptions
    if (db_1.db.subscriptions.count() === 0) {
        const subscriptions = [
            {
                id: 'sub-pat-1',
        ...
        db_1.db.subscriptions.insertMany(subscriptions);
    }
    // C. Organization Licenses
    if (db_1.db.organizationLicenses.count() === 0) {
        const licenses = [
            {
                id: 'lic-1',
                licenseId: 'LIC10001',
        ...
        db_1.db.organizationLicenses.insertMany(licenses);
    }
    // D. Usage Records
    if (db_1.db.usageRecords.count() === 0) {
        const usageRecords = [
            {
                id: 'usg-1',
                organizationId: 'org-1',
        ...
        db_1.db.usageRecords.insertMany(usageRecords);
    }
    // E. Medication Supplies
    if (db_1.db.medicationSupplies.count() === 0) {
        const supplies = [
            {
                id: 'sup-1',
                patientId: 'PAT10001',
        ...
        db_1.db.medicationSupplies.insertMany(supplies);
    }
    // F. Refill Requests
    if (db_1.db.refillRequests.count() === 0) {
        ...
        db_1.db.refillRequests.insertMany(refillRequests);
    }
    // G. Hospitals & Organizations check
    const org1 = db_1.db.organizations.findById('org-1');
    if (org1) {
        db_1.db.organizations.update('org-1', {
            name: 'ABC Hospital',
            hospitalId: 'HOSP-00125',
            type: 'HOSPITAL'
        });
    }
    const org2 = db_1.db.organizations.findById('org-2');
    if (org2) {
        db_1.db.organizations.update('org-2', {
            name: 'XYZ Hospital',
            hospitalId: 'HOSP-00482',
            type: 'HOSPITAL'
        });
    ...
    const docRavi = db_1.db.doctors.findOne(d => d.doctorId === 'DOC10001');
    if (docRavi) {
        db_1.db.doctors.update(docRavi.id, {
            councilRegistrationId: 'DOC-AP-123456',
            hospitalId: 'HOSP-00125',
            hospitalName: 'ABC Hospital'
        });
    }
    const docSneha = db_1.db.doctors.findOne(d => d.doctorId === 'DOC10002');
    if (docSneha) {
        db_1.db.doctors.update(docSneha.id, {
            councilRegistrationId: 'DOC-TS-789012',
            hospitalId: 'HOSP-00482',
            hospitalName: 'XYZ Hospital'
        });
    }
    const docAmit = db_1.db.doctors.findOne(d => d.doctorId === 'DOC10003');
    if (docAmit) {
        db_1.db.doctors.update(docAmit.id, {
            councilRegistrationId: 'DOC-MH-345678',
            hospitalId: 'HOSP-00125',
            hospitalName: 'ABC Hospital'
        });
    ...
    // I. Receptionists
    if (db_1.db.receptionists.count() === 0) {
        const passwordHash = bcryptjs_1.default.hashSync('password123', 10);
        // Ensure Users exist for receptionists
        if (!db_1.db.users.findOne(u => u.email === 'receptionist.abc@medsafe.local')) {
            db_1.db.users.insert({
                id: 'usr-rec-1',
                email: 'receptionist.abc@medsafe.local',
                passwordHash,
                role: 'RECEPTIONIST',
        ...
        if (!db_1.db.users.findOne(u => u.email === 'receptionist.xyz@medsafe.local')) {
            db_1.db.users.insert({
                id: 'usr-rec-2',
                email: 'receptionist.xyz@medsafe.local',
                passwordHash,
        ...
        db_1.db.receptionists.insertMany([
            {
                id: 'rec-1',
                receptionistId: 'REC-0018',
                userId: 'usr-rec-1',
    ...
    const existingRecUser = db_1.db.users.findOne(u => u.email === 'receptionist.abc@medsafe.local');
    if (!existingRecUser) {
        const saltSync = bcryptjs_1.default.genSaltSync(10);
        const recHash = bcryptjs_1.default.hashSync('DemoPass123!', saltSync);
        db_1.db.users.insertMany([
            {
                id: 'usr-rec-1',
                email: 'receptionist.abc@medsafe.local',
    ...
    if (db_1.db.patientHospitalRelationships.count() === 0) {
        db_1.db.patientHospitalRelationships.insertMany([
            {
                id: 'phr-1',
                relationshipId: 'REL-HOSP-10001',
                patientId: 'PAT10001',
    ...
    if (db_1.db.doctorPatientRelationships.count() === 0) {
        db_1.db.doctorPatientRelationships.insertMany([
            {
                id: 'dpr-1',
                relationshipId: 'REL-DOC-10001',
                patientId: 'PAT10001',
    ...
    const reqs = db_1.db.consultationRequests.find();
    for (const r of reqs) {
        if (!r.hospitalId) {
            const doc = db_1.db.doctors.findOne(d => d.doctorId === r.doctorId);
            db_1.db.consultationRequests.update(r.id, {
