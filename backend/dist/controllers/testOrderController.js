...
exports.testOrderController = exports.TEST_PRICING = void 0;
const uuid_1 = require("uuid");
const db_1 = require("../database/db");
// Standard pricing lookup for common diagnostic tests
exports.TEST_PRICING = {
...
exports.testOrderController = {
    // Doctor or Clinician prescribes a test order
    createOrder: async (req, res) => {
        try {
            const { patientId, testName, testCategory, reason, priority = 'ROUTINE', labId, customCost, notes } = req.body;
            ...
            const doctor = db_1.db.doctors.findOne(d => d.doctorId === doctorId);
            const doctorName = doctor ? doctor.name : (req.user?.name || 'Dr. Sarah Jenkins');
            // Select lab
            let assignedLab = labId ? db_1.db.labs.findOne(l => l.labId === labId) : null;
            if (!assignedLab) {
                assignedLab = db_1.db.labs.findOne(l => l.verificationStatus === 'VERIFIED') || db_1.db.labs.find()[0];
            }
            // Generate sequential Test Order ID
            const allOrders = db_1.db.testOrders.find();
            ...
            const allBills = db_1.db.bills.find();
            ...
            const newBill = {
                id: `bil-${(0, uuid_1.v4)().substring(0, 8)}`,
                billId,
                patientId,
                patientName: patient.name,
            ...
            db_1.db.bills.insert(newBill);
            newOrder.billId = billId;
            db_1.db.testOrders.insert(newOrder);
            // Patient Notification
            const patNotification = {
            ...
            db_1.db.notifications.insert(patNotification);
            // Lab Notification if lab user exists
            if (assignedLab) {
                const labUser = db_1.db.users.findOne(u => u.labId === assignedLab?.labId || u.id === assignedLab?.userId);
                if (labUser) {
                    ...
                    db_1.db.notifications.insert(labNotification);
                }
            }
            res.status(201).json({
                success: true,
    ...
    getPatientOrders: async (req, res) => {
        try {
            const patientId = req.params.patientId;
            const orders = db_1.db.testOrders.find(o => o.patientId === patientId).sort((a, b) => b.orderedAt.localeCompare(a.orderedAt));
    ...
    getAllOrders: async (req, res) => {
        try {
            ...
            let orders = db_1.db.testOrders.find().sort((a, b) => b.orderedAt.localeCompare(a.orderedAt));
            if (status) {
    ...
    getOrderById: async (req, res) => {
        try {
            ...
            const order = db_1.db.testOrders.findOne(o => o.id === id || o.testOrderId === id);
            if (!order) {
    ...
    collectSample: async (req, res) => {
        try {
            ...
            const now = new Date().toISOString();
            const updated = db_1.db.testOrders.update(order.id, {
    ...
    updateStatus: async (req, res) => {
        try {
            ...
            const updated = db_1.db.testOrders.update(order.id, updates);
    ...
    getCatalogue: async (req, res) => {
        try {
            const items = Object.entries(exports.TEST_PRICING).map(([name, info]) => ({
                testName: name,
