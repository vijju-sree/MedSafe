...
exports.receptionistController = {
    ...
    getDashboard: async (req, res) => {
        try {
            ...
            const org = db_1.db.organizations.findOne(o => o.hospitalId === hospitalId || o.id === hospitalId);
            const hospitalName = org?.name || user.hospitalName || 'ABC Hospital';
            // 1. Hospital Patients count
            const hospitalPatientRels = db_1.db.patientHospitalRelationships.find(r => r.hospitalId === hospitalId && r.status === 'ACTIVE');
            const patientIds = Array.from(new Set(hospitalPatientRels.map(r => r.patientId)));
            // 2. Pending Requests for this hospital
            const pendingRequests = db_1.db.consultationRequests.find(r => (r.hospitalId === hospitalId || (!r.hospitalId && hospitalId === 'HOSP-00125')) && r.status === 'PENDING');
            // 3. Hospital Doctors
            const hospitalDoctors = db_1.db.doctors.find(d => Boolean(d.hospitalId === hospitalId || (org && d.organizationId === org.id)));
            // 4. Reports for this hospital's patients
            const hospitalReports = db_1.db.medicalReports.find(r => patientIds.includes(r.patientId));
            // 5. Test orders for this hospital
            const hospitalOrders = db_1.db.testOrders.find(o => patientIds.includes(o.patientId));
            // 6. Recent requests
            const allHospitalRequests = db_1.db.consultationRequests.find(r => r.hospitalId === hospitalId || (!r.hospitalId && hospitalId === 'HOSP-00125')).sort((a, b) => b.requestedAt.localeCompare(a.requestedAt));
            res.json({
                success: true,
                data: {
                    hospital: {
    ...
    getRequests: async (req, res) => {
        try {
            ...
            let requests = db_1.db.consultationRequests.find(r => r.hospitalId === hospitalId || (!r.hospitalId && hospitalId === 'HOSP-00125'));
    ...
    respondToRequest: async (req, res) => {
        try {
            ...
            const request = db_1.db.consultationRequests.findById(id);
            if (!request) {
            ...
            if (action === 'ACCEPTED') {
                const doctor = db_1.db.doctors.findOne(d => d.doctorId === request.doctorId || d.id === request.doctorId);
                const patient = db_1.db.patients.findOne(p => p.patientId === request.patientId);
                if (patient) {
                    // 1. Create Patient-Hospital Relationship
                    const existingPhr = db_1.db.patientHospitalRelationships.findOne(r => r.patientId === patient.patientId && r.hospitalId === hospitalId);
                    if (!existingPhr) {
                        db_1.db.patientHospitalRelationships.insert({
                            id: `phr-${(0, uuid_1.v4)().substring(0, 8)}`,
                        ...
                        db_1.db.patientHospitalRelationships.update(existingPhr.id, { status: 'ACTIVE' });
                    }
                    // 2. Create Doctor-Patient Relationship
                    if (doctor) {
                        const existingDpr = db_1.db.doctorPatientRelationships.findOne(r => r.patientId === patient.patientId && r.doctorId === doctor.doctorId && r.hospitalId === hospitalId);
                        if (!existingDpr) {
                            db_1.db.doctorPatientRelationships.insert({
                                id: `dpr-${(0, uuid_1.v4)().substring(0, 8)}`,
                            ...
                            db_1.db.doctorPatientRelationships.update(existingDpr.id, {
                                status: 'ACTIVE',
                                acceptedBy: user?.receptionistId || 'REC-0018',
                                acceptedAt: now
                            });
                        ...
                        db_1.db.consents.insert(consent);
                        ...
                        db_1.db.consultations.insert(consultation);
                        // Notify Doctor
                        const docUser = db_1.db.users.findOne(u => u.doctorId === doctor.doctorId || u.id === doctor.userId);
                        if (docUser) {
                            db_1.db.notifications.insert({
                                id: `notif-doc-linked-${(0, uuid_1.v4)().substring(0, 8)}`,
                    ...
                    // Notify Patient
                    const patientUser = db_1.db.users.findOne(u => u.patientId === patient.patientId);
                    if (patientUser) {
                        db_1.db.notifications.insert({
                            id: `notif-pat-acc-${(0, uuid_1.v4)().substring(0, 8)}`,
    ...
    getHospitalPatients: async (req, res) => {
        try {
            ...
            const hospitalRels = db_1.db.patientHospitalRelationships.find(r => r.hospitalId === hospitalId && r.status === 'ACTIVE');
            const patientIdSet = new Set(hospitalRels.map(r => r.patientId));
            let patients = db_1.db.patients.find(p => patientIdSet.has(p.patientId));
            if (q) {
            ...
            const enriched = patients.map(p => {
                const dprs = db_1.db.doctorPatientRelationships.find(r => r.patientId === p.patientId && r.hospitalId === hospitalId && r.status === 'ACTIVE');
    ...
    getPatientDetail: async (req, res) => {
        try {
            ...
            const rel = db_1.db.patientHospitalRelationships.findOne(r => r.patientId === patientId && r.hospitalId === hospitalId && r.status === 'ACTIVE');
            ...
            const assignedDoctors = db_1.db.doctorPatientRelationships.find(r => r.patientId === patientId && r.hospitalId === hospitalId && r.status === 'ACTIVE');
            const testOrders = db_1.db.testOrders.find(o => o.patientId === patientId);
            const reports = db_1.db.medicalReports.find(r => r.patientId === patientId);
            // Medication operational overview (Read-only summary)
            const medications = db_1.db.medications.find(m => m.patientId === patientId && m.active).map(m => ({
                name: m.name,
            ...
            // Consultations
            const consultations = db_1.db.consultations.find(c => c.patientId === patientId);
            // Bills
            const bills = db_1.db.bills.find(b => b.patientId === patientId);
            ...
            res.json({
                ...
                data: {
                    ...
                    assignedDoctors: assignedDoctors.map(d => {
                        const docInfo = db_1.db.doctors.findOne(doc => doc.doctorId === d.doctorId || doc.id === d.doctorId);
                        return {
                            doctorId: d.doctorId,
                            doctorName: d.doctorName || docInfo?.name || 'Dr. Attending',
                            name: d.doctorName || docInfo?.name || 'Dr. Attending',
                            specialization: docInfo?.specialty || docInfo?.specialization || 'General Medicine',
    ...
    searchGlobalPatient: async (req, res) => {
        try {
            ...
            const patient = db_1.db.patients.findOne(p => p.patientId.toUpperCase() === q.toUpperCase() ||
            ...
            const existingRel = db_1.db.patientHospitalRelationships.findOne(r => r.patientId === patient.patientId && r.hospitalId === hospitalId && r.status === 'ACTIVE');
            const patData = {
                patientId: patient.patientId,
                name: patient.name,
                gender: patient.gender,
    ...
    linkPatient: async (req, res) => {
        try {
            ...
            let phr = db_1.db.patientHospitalRelationships.findOne(r => r.patientId === patient.patientId && r.hospitalId === hospitalId);
            if (!phr) {
                phr = db_1.db.patientHospitalRelationships.insert({
                    id: `phr-${(0, uuid_1.v4)().substring(0, 8)}`,
                    relationshipId: `REL-HOSP-${Date.now()}`,
                    patientId: patient.patientId,
                    hospitalId,
                ...
                db_1.db.patientHospitalRelationships.update(phr.id, {
                    status: 'ACTIVE',
                    receptionistId: user?.receptionistId,
                    endedAt: undefined
                });
            ...
            db_1.db.patients.update(patient.id, { clinicIds: updatedClinicIds });
            ...
            if (doctorId) {
                ...
                if (doctor) {
                    doctorName = doctor.name;
                    let dpr = db_1.db.doctorPatientRelationships.findOne(r => r.patientId === patient.patientId && r.doctorId === doctor.doctorId && r.hospitalId === hospitalId);
                    if (!dpr) {
                        db_1.db.doctorPatientRelationships.insert({
                            id: `dpr-${(0, uuid_1.v4)().substring(0, 8)}`,
                            relationshipId: `REL-DOC-${Date.now()}`,
                    ...
                    const updatedDoctorIds = Array.from(new Set([...(patient.doctorIds || []), doctor.doctorId]));
                    db_1.db.patients.update(patient.id, { doctorIds: updatedDoctorIds });
                    // Add consent
                    db_1.db.consents.insert({
                        id: `con-${(0, uuid_1.v4)().substring(0, 8)}`,
                        patientId: patient.patientId,
                        granteeId: doctor.userId,
                        granteeName: doctor.name,
            ...
            if (patientUser) {
                db_1.db.notifications.insert({
                    id: `notif-pat-reg-${(0, uuid_1.v4)().substring(0, 8)}`,
                    recipientId: patientUser.id,
                    recipientRole: 'PATIENT',
                    patientId: patient.patientId,
                    type: 'FOLLOW_UP',
    ...
    getHospitalReports: async (req, res) => {
        try {
            ...
            const reports = db_1.db.medicalReports.find(r => patientIds.includes(r.patientId)).sort((a, b) => b.uploadedAt.localeCompare(a.uploadedAt));
    ...
    getHospitalReportDetail: async (req, res) => {
        try {
            const user = req.user;
            const { id } = req.params;
            const hospitalId = user?.hospitalId || 'HOSP-00125';
            const report = db_1.db.medicalReports.findOne(r => r.id === id || r.reportId === id);
            if (!report) {
            ...
            const rel = db_1.db.patientHospitalRelationships.findOne(r => r.patientId === report.patientId && r.hospitalId === hospitalId && r.status === 'ACTIVE');
            if (!rel && user?.role !== 'ADMIN') {
                res.status(403).json({
                    success: false,
    ...
    getHospitalDoctors: async (req, res) => {
        try {
            ...
            const hospitalId = user?.hospitalId || 'HOSP-00125';
            const org = db_1.db.organizations.findOne(o => o.hospitalId === hospitalId || o.id === hospitalId);
            const doctors = db_1.db.doctors.find(d => Boolean(d.hospitalId === hospitalId || (org && d.organizationId === org.id))).map(d => ({
                id: d.id,
                doctorId: d.doctorId,
                name: d.name,
                specialty: d.specialty,
