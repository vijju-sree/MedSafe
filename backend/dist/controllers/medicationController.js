...
exports.medicationController = {
    ...
    getMedicationById: async (req, res) => {
        try {
            const { id } = req.params;
            const medication = db_1.db.medications.findById(id);
            if (!medication) {
                res.status(404).json({ success: false, message: 'Medication not found.' });
                return;
            }
            const history = db_1.db.medicationPlanHistory.find(h => h.medicationId === id);
    ...
    addMedication: async (req, res) => {
        try {
            const { patientId, prescriptionId, doctorId, doctorName, name, genericName, strength, dosageAmount, dosageUnit, form, route, frequency, scheduleTimes, startDate, endDate, instructions, foodInstruction, totalQuantity, refillDate } = req.body;
            ...
            db_1.db.medications.insert(newMedication);
            // Record Plan History (Initial Creation)
            const historyRecord = {
                id: `mph-${(0, uuid_1.v4)().substring(0, 8)}`,
                patientId,
            ...
            db_1.db.medicationPlanHistory.insert(historyRecord);
            ...
            db_1.db.medicationSupplies.insert(supplyRecord);
            // Link to prescription if prescriptionId provided
            if (newMedication.prescriptionId && newMedication.prescriptionId !== 'RX-MANUAL') {
                const rx = db_1.db.prescriptions.findOne(p => p.id === newMedication.prescriptionId || p.prescriptionId === newMedication.prescriptionId);
                if (rx) {
                    const currentMedIds = rx.medicationIds || [];
                    if (!currentMedIds.includes(newMedication.id)) {
                        db_1.db.prescriptions.update(rx.id, {
            ...
            if (user) {
                db_1.db.notifications.insert({
                    id: `notif-newmed-${(0, uuid_1.v4)().substring(0, 8)}`,
                    recipientId: user.id,
    ...
    updateMedicationPlan: async (req, res) => {
        try {
            const { id } = req.params;
            const { reason, ...updates } = req.body;
            const existingMed = db_1.db.medications.findById(id);
            if (!existingMed) {
                res.status(404).json({ success: false, message: 'Medication not found.' });
                return;
            }
            ...
            const updatedMed = db_1.db.medications.update(id, {
                ...updates,
                version: newVersion,
                updatedAt: now.toISOString()
            });
            ...
            db_1.db.medicationPlanHistory.insert(historyRecord);
            // Notify Patient
            const patient = db_1.db.patients.findOne(p => p.patientId === existingMed.patientId);
            const user = patient ? db_1.db.users.findOne(u => u.patientId === patient.patientId) : undefined;
            if (user) {
                db_1.db.notifications.insert({
                    id: `notif-planup-${(0, uuid_1.v4)().substring(0, 8)}`,
                    recipientId: user.id,
                    recipientRole: 'PATIENT',
                    patientId: existingMed.patientId,
                    type: 'MEDICATION_PLAN_CHANGED',
            ...
            const safetyAlerts = safetyService_1.safetyService.evaluateAll(existingMed.patientId);
            res.json({
                success: true,
                data: {
    ...
    discontinueMedication: async (req, res) => {
        try {
            ...
            const med = db_1.db.medications.findById(id);
            if (!med) {
                res.status(404).json({ success: false, message: 'Medication not found.' });
                return;
            }
            const updated = db_1.db.medications.update(id, { active: false, prescriptionStatus: 'DISCONTINUED' });
            // Record Plan History
            db_1.db.medicationPlanHistory.insert({
                id: `mph-${(0, uuid_1.v4)().substring(0, 8)}`,
