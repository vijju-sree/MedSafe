...
exports.reportController = {
    ...
    uploadReport: async (req, res) => {
        try {
            const { testOrderId, resultSummary, detailedFindings, referenceRange, status = 'FINAL', performedBy, fileData, fileName, fileType = 'text/plain' } = req.body;
            ...
            const order = db_1.db.testOrders.findOne(o => o.id === testOrderId || o.testOrderId === testOrderId);
            if (!order) {
                res.status(404).json({ success: false, message: `Test order ${testOrderId} not found.` });
                return;
            }
            // Generate sequential report ID
            const allReports = db_1.db.medicalReports.find();
            ...
            const newReport = {
                id: `rpt-${(0, uuid_1.v4)().substring(0, 8)}`,
                reportId,
            ...
            db_1.db.medicalReports.insert(newReport);
            // Update test order status to COMPLETED
            db_1.db.testOrders.update(order.id, {
                status: 'COMPLETED',
                completedAt: now,
                reportId: newReport.id
            });
            // Notify Patient
            const patient = db_1.db.patients.findOne(p => p.patientId === order.patientId);
            if (patient) {
                const patNotif = {
                ...
                db_1.db.notifications.insert(patNotif);
            }
            // Notify Prescribing Doctor
            const doctorUser = db_1.db.users.findOne(u => u.doctorId === order.doctorId);
            if (doctorUser) {
                const docNotif = {
                    id: `notif-${(0, uuid_1.v4)().substring(0, 8)}`,
                ...
                db_1.db.notifications.insert(docNotif);
            }
            res.status(201).json({
                success: true,
                data: {
    ...
    getPatientReports: async (req, res) => {
        try {
            ...
            if (user) {
                ...
                if (user.role === 'DOCTOR') {
                    ...
                    if (doc && !doc.authorizedPatientIds?.includes(patientId)) {
                        ...
                        const hasOrder = db_1.db.testOrders.findOne(o => o.doctorId === user.doctorId && o.patientId === patientId);
                        const hasReq = db_1.db.consultationRequests.findOne(c => c.doctorId === user.doctorId && c.patientId === patientId && c.status === 'ACCEPTED');
                        if (!hasOrder && !hasReq) {
                ...
                if (user.role === 'CAREGIVER') {
                    const consent = db_1.db.consents.findOne(c => c.granteeId === user.id && c.patientId === patientId && c.status === 'ACTIVE');
                    if (!consent) {
                ...
                if (user.role === 'RECEPTIONIST') {
                    const rel = db_1.db.patientHospitalRelationships.findOne(r => r.patientId === patientId && r.hospitalId === user.hospitalId && r.status === 'ACTIVE');
                    if (!rel) {
            ...
            const reports = db_1.db.medicalReports.find(r => r.patientId === patientId).sort((a, b) => b.uploadedAt.localeCompare(a.uploadedAt));
    ...
    getReportById: async (req, res) => {
        try {
            ...
            if (user && user.role === 'RECEPTIONIST') {
                const rel = db_1.db.patientHospitalRelationships.findOne(r => r.patientId === report.patientId && r.hospitalId === user.hospitalId && r.status === 'ACTIVE');
                if (!rel) {
                    res.status(403).json({
                        success: false,
...
`.trim();
    ...
    getAllReports: async (req, res) => {
        try {
            const reports = db_1.db.medicalReports.find().sort((a, b) => b.uploadedAt.localeCompare(a.uploadedAt));
            res.json({ success: true, data: { reports } });
        }
        catch (error) {
            res.status(500).json({ success: false, message: error.message });
